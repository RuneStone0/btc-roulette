...
async def interact(conn, svr, connector, method, args, verbose=False):
    ...
    try:
        if method.startswith('address.'):
            # Use ElectrumQuery for address-related queries
            client = ElectrumQuery(svr.hostname, svr.ports[0])
            if method == 'address.get_balance':
                rv = client.get_address_balance(args[0])
            else:
